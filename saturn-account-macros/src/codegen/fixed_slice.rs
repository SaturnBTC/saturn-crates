    ...
    if is_acc_info_elem {
        ...
        if cfg.seeds.is_some() {
            ...
            quote! {
                ...
                let #ident = &accounts[slice_start .. slice_end];
                idx = slice_end;
            }
        } else {
            quote! {
...
mod tests {
    ...
    fn generates_fixed_slice_non_account_info() {
        let di: DeriveInput = parse_quote! {
            struct Accs<'info> {
                #[account(len = 4)]
                vec_borsh: Vec<saturn_account_parser::codec::BorshAccount<'info, u64>>,
            }
        };
        let parsed = crate::parser::parse_fields(extract_named_fields(&di)).expect("parse ok");
        let cfg = &parsed[0];
        ...
        let rendered = ts.to_string();
        assert!(rendered.contains("& accounts") || rendered.contains("&accounts["));
        // Should not allocate Vec clone path.
    ...
    fn generates_fixed_slice_account_pda_non_account_info() {
        ...
        let rendered = ts.to_string();
        assert!(rendered.contains("get_indexed_pda_account"));
        // Non AccountInfo slice should borrow the accounts slice, not clone into a Vec.
        assert!(rendered.contains("& accounts") || rendered.contains("&accounts["));
        assert!(!rendered.contains("clone()"));
    }
