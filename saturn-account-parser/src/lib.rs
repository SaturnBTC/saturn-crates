...
/// reference Borsh traits even if the user renamed the `borsh` crate.
///
/// The macros expect both `saturn_account_parser::__private::borsh` and
/// `saturn_account_parser::__private::__borsh` to be valid paths. We therefore
/// re-export the traits under both names.
pub mod __private {
    /// `saturn_account_parser::__private::borsh` – used at runtime when
    /// deserializing instruction data.
    pub mod borsh {
        pub use borsh::{BorshDeserialize, BorshSerialize};
    }

    /// `saturn_account_parser::__private::__borsh` – used inside `#[derive]`
    /// annotations generated by the macros.
    pub mod __borsh {
        pub use borsh::{BorshDeserialize, BorshSerialize};
    }
}
